...
\begin{lstlisting}[language=json, caption={Esempio di richiesta AddKV per aggiungere un Asset}]
    ...
    ||"value"||: {
        ...
        ||"picture"||: "Screenshot from 2026-02-26 15-50-08.png",
        ||"price"||: 50000,
        ||"size"||: 120,
        ||"status"||: "active",
        ||"title"||: "Villa"
...
\begin{table}[h!]
    ...
    \begin{tabular}{lll}
        ...
        \texttt{name} & string & \texttt{nome utente} \\
        \texttt{surname} & string & \texttt{cognome utente} \\
        \texttt{email} & string & \texttt{indirizzo email} \\
        \texttt{birthday} & date & \texttt{data di nascita} \\
        \texttt{cellularNumber} & number & \texttt{numero di cellulare} \\
        \texttt{passwordHash} & string & \texttt{hash della password} \\
        \texttt{lastLoginAt} & int (epoch) &  \texttt{ultima data di login} \\
        \texttt{createdAt} & int (epoch) &  \texttt{data di creazione account} \\
        \hline
    \end{tabular}
