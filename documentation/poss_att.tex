...
\subsection*{Contromisure Prioritarie su Endpoint Critici (STRIDE)}
In aggiunta alle difese generali, la \cref{tab:stride-mitigations} sintetizza le mitigazioni operative da applicare agli endpoint a maggiore esposizione.

\begin{table}[!htbp]
    \centering
    \small
    \begin{tabular}{@{}p{0.32\textwidth}p{0.20\textwidth}p{0.34\textwidth}p{0.10\textwidth}@{}}
        \toprule
        \textbf{Endpoint} & \textbf{Categoria STRIDE} & \textbf{Contromisura consigliata} & \textbf{Priorita} \\
        \midrule
        \url{/api/v1/auth/login} (POST) & Spoofing, DoS & rate limiting + lockout progressivo + logging IP & Alta \\
        \url{/api/v1/bids} (POST) & Tampering, DoS & validazione server-side rigorosa + anti flood per asta & Alta \\
        \url{/api/v1/auctions/getAuctionHistory/\{id\}} (GET) & Repudiation, Disclosure & audit log immutabile + filtri ruolo/stato & Alta \\
        \url{/api/v1/auth/profile} (GET/PUT) & Disclosure, Tampering & whitelist campi + validazione formati + JWT valido & Media-Alta \\
        \url{/api/v1/admin/*} & Elevation of Privilege & enforcement RBAC stretto + alert su accessi anomali & Critica \\
        \bottomrule
    \end{tabular}
    \caption{Mitigazioni endpoint-centric basate su STRIDE}
    \label{tab:stride-mitigations}
\end{table}

Per gli endpoint di stato asta (\url{/api/v1/auctions/lock/\{id\}} e \url{/api/v1/auctions/close/\{id\}}) e inoltre consigliata una policy di invocazione autenticata e autorizzata, con tracciamento completo dei chiamanti, poiche tali operazioni influenzano direttamente l'integrita del risultato d'asta.
